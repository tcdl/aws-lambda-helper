...
function saveRecord (params, callback) {
  const filtered = params.items.filter((item) => {
    return item.url; // only save an object with a url prop.
  });
...
AwsHelper.pushResultToClient = function (params, callback) {
  if (!params.id) {
   ...
   // first save the item to S3 so we are certain that it's retrievable
  saveRecord(params, function (err, json) { // see: https://git.io/vrpYN
    if (err) {
       AwsHelper.log.error({err: err, data: json}, 'ERROR Saving Data to S3');
     }
...
// Add other libs
AwsHelper.searchTerms = require('./searchTerms');
